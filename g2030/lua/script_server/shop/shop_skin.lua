...
function M:init()
    local config1 = T(Config, "SkinConfig")
    shopbase.init(self, Define.TabType.Skin, config1)
end
...
function M:setPlayerBuyInfo(player, buyInfo)
    player:setSkin(buyInfo)
end

function M:onPlayerUseItem(player, item)
    player:changeActor(item.actor)
    print("onPlayerUseItem 玩家 : "..tostring(player.name).. "self.type ：", tostring(self.type).." item.s_actor : ".. tostring(item.actor))
end

function M:onPlayerUseDefaultItem(player)
    print("onPlayerUseItem 玩家 : "..tostring(player.name).. "self.type ：", tostring(self.type))
end
...
function M:onUnload(player, itemId)
    local item = self.config:getItemById(itemId)
    local buyInfo = self:getPlayerBuyInfo(player)
    buyInfo[tostring(item.id)] = BuyStatus.Buy
